...
function is_allowed_to_access_attribute(obj, values, attribute)
    if not values.type or not global.lookup.subclasses[obj.object_name] or not global.lookup.subclasses[obj.object_name][attribute] then
        return true
    end
    local subclasses = global.lookup.subclasses[obj.object_name][attribute]
    if type(subclasses) == 'string' then
        return subclasses == obj.type
    else
